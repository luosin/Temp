													 ...
													 double scale, double rotation )
{
	...
	SetScaleFactor(pBlkRef, scale);
	pBlkRef->setRotation(rotation);
// 	CBlockUtil::SetBlockRefAttribute(pBlkRef,_T("TUNAME_001"),_T("泰肥线光缆定测图-天平店通信站"));
// 	CBlockUtil::SetBlockRefAttribute(pBlkRef,_T("TUNAME"),_T("泰肥线光缆定测图-天平店通信站"));
// 	CBlockUtil::SetBlockRefAttribute(pBlkRef,_T("TUHAO_001"),_T("泰肥线光缆定测图-天平店通信站"));
	// 判断指定的块表记录是否包含属性定义
...
AcGePoint2d CBlockUtil::GetBlockRefPosition( AcDbBlockReference *pBlkRef )
{
	AcGePoint2d position;
	position =CConvertUtil::ToPoint2d(pBlkRef->position());
	return position;
}
