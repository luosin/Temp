...
{	
	double angle = 2 * CMathUtil::GetPI() / (double)number;
	AcGePoint2dArray points;
	...
	if (acdbOpenAcDbEntity(pEnt, polyId, AcDb::kForRead) == Acad::eOk)
	{
...
bool CPolylineUtil::InsertWithTwoPolyline( AcDbPolyline *pPoly1,AcDbPolyline*pPoly2,AcGePoint2dArray &point2ds,double tol )
{
	bool result =false;
	for(int i=0;i<(int)pPoly1->numVerts();i++)
	{
		AcGePoint3d fvert;
		pPoly1->getPointAt(i,fvert);
		if(CPolylineUtil::PointIsPolyVert(pPoly2,CConvertUtil::ToPoint2d(fvert),tol))
		{
			point2ds.append(CConvertUtil::ToPoint2d(fvert));
			result=true;
		}
