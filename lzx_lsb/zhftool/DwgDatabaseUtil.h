...
public:
	...
	static AcDbObjectIdArray GetAll_Polyline_EntityIds(const TCHAR* layerName = NULL, 
		AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase());
	static AcDbObjectIdArray GetAll_Block_EntityIds(const TCHAR* layerName = NULL, 
		AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase());
	static AcDbObjectIdArray GetBlockids_byName(const TCHAR* blockName,const TCHAR* layerName = NULL, 
		AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase());
///	AcDbObjectIdArray GetAll_Polyline_EntityIds( const TCHAR* layerName /*= NULL*/, AcDbDatabase *pDb /*= acdbHostApplicationServices()->workingDatabase()*/ );
	// 获得整个图形模型空间所有实体的包围框
