...
void CNeuListCtrl::SetItemControl(enCONTROLTYPE enControlType,int iIndex)
{
	...
	switch(enControlType)
	{
		case EDITBOX:
			...
			((CEdit*)pSubItemInfo->m_pWnd)->Create (dwStyle,ControlSize,this ,m_iControlIDCounter);	
			break;
		case CHECKBOX:
			pSubItemInfo->m_pWnd = new CButton();
			((CButton*)pSubItemInfo->m_pWnd)->Create (_T(""),dwStyle|BS_AUTOCHECKBOX,ControlSize,this ,m_iControlIDCounter);	
		...
		case COMBOBOX:
			...
			//((CComboBox*)pSubItemInfo->m_pWnd)->SetItemHeight(1, 500) ;
			break;
		case LISTBOX:
		...
		case BUTTON:
			pSubItemInfo->m_pWnd = new CButton();
			((CButton*)pSubItemInfo->m_pWnd)->Create (_T(""),dwStyle,ControlSize,this ,m_iControlIDCounter);
			break;
	}
